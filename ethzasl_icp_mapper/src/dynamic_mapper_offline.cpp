	...
	eps(0.0001)
{
  ...
  if (bag_file_mode || true)
  {
    ...
    for (const rosbag::MessageInstance& msg : view)
    {
      ...
      if (msg.isType<sensor_msgs::PointCloud2>()) //if (m.getTopic() == velodyne_packet_topic)
      {
        //ROS_INFO_STREAM("bag_file_mode decode");
        sensor_msgs::PointCloud2ConstPtr velo_point_ptr = msg.instantiate<sensor_msgs::PointCloud2>();
        if (velo_point_ptr != NULL)
        {
          ...
          sensor_msgs::PointCloud2 velo_point = *velo_point_ptr;
           ROS_INFO_STREAM(percent_complete<<" % complete, "<<complete_secs<<"seconds of Duration: "<<duration<<" seconds | stamp = "<<velo_point.header.stamp.toSec());
           if(velo_point.width<100)//even 15% random sample should give more than 15 points..  360 degree could shound have much more in general
           {
			   bag_file_interleave_step=bag_file_interleave;
			   ROS_WARN_STREAM("skipping current point due to very small number of points "<<velo_point.width<<" at stamp = "<<velo_point.header.stamp.toSec());
			   continue;
		   }
           
          unique_ptr<DP> cloud(new DP(PointMatcher_ros::rosMsgToPointMatcherCloud<float>(velo_point)));
     	  //processCloud(move(cloud), velo_point.header.frame_id, velo_point.header.stamp, velo_point.header.seq);
           processCloud(move(cloud), velo_point.header.frame_id, ros::Time::now(), velo_point.header.seq);
          }//else ROS_INFO_STREAM("skipping bag_file_interleave_step "<<bag_file_interleave_step <<" bag_file_interleave "<<bag_file_interleave);
          
           if(bag_file_save_interval>0 && ++bag_file_save_interval_step>=bag_file_save_interval)
          {
            bag_file_save_interval_step=0;
            bag_file_save_interval_count++;
            mapPointCloud->save(filePath + "/" + std::to_string(bag_file_save_interval_count) + finalMapName);
          }
          ros::spinOnce();
        ...
        if (temp_percent_complete >= percent_complete + 1)
        {
          ...
          ROS_INFO_STREAM_THROTTLE(1.0, percent_complete<<" % complete, "<<complete_secs<<"seconds of Duration: "<<duration<<" seconds ");
        }
      }
    }
...
void Mapper::processCloud(unique_ptr<DP> newPointCloud, const std::string& scannerFrame, const ros::Time& stamp, uint32_t seq)
{
	...
	{
		ROS_WARN_STREAM("[ICP] Got part of clouds has " << newPointCloud->times.cols() << "Columns. " << e.what());
	}
	if (goodCount == 0)
	{
	...
	{
		ROS_INFO_STREAM("[ICP] Not enough points in newPointCloud: only " << ptsCount << " pts.");
		return;
	}
	...
	try 
	{
		...
		{
			odomPub.publish(PointMatcher_ros::eigenMatrixToOdomMsg<float>(T_updatedScanner_to_map, mapFrame, stamp, sensorFrame));
		}
		// Publish odometry ir robot frame
		if (robotPub.getNumSubscribers())
		{
			robotPub.publish(PointMatcher_ros::eigenMatrixToOdomMsg<float>(T_robot_to_map, mapFrame, stamp, robotFrame));//TODO
		}
		// Publish error on odometry
		if (odomErrorPub.getNumSubscribers())
			odomErrorPub.publish(PointMatcher_ros::eigenMatrixToOdomMsg<float>(T_odom_to_map, mapFrame, stamp, sensorFrame ));
...
void Mapper::setMap(DP* newMapPointCloud)
{
	...
	ROS_INFO_STREAM("[MAP] number of subs = " << mapPub.getNumSubscribers() << " mapFrame" << mapFrame);
